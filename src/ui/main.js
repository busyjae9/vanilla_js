...
MainUI.initPipe = () => go(Data.todayTodo, MainUI.init);
...
MainUI.mkConAndSave = () =>
    go(
        $qs(".input__input_box"),
        $children,
        tap(Data.addTodo, (todoData) =>
            todoData.date == Data.date ? MainUI.mkCon(todoData) : null
        ),
...
MainUI.rmAllAndDel = () =>
    go($qs(".contents"), MainUI.rmAll, Data.removeAllTodoData);
...
MainUI.delegate = (container_el) =>
    go(
        ...
        $delegate("change", ".header__today", ({currentTarget}) =>
            go(currentTarget, MainUI.updateElValue(Data.updateDay), () =>
                MainUI.contentViewUpdate()
            )
        ),
