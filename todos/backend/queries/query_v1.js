import { COLUMN, EQ, QUERY, QUERY1, SET, TB, VALUES } from '../db/db_connect.js';
...
Query.success = curry2((res, message, data) =>
    res.status(200).json({
        code: '0001',
        result: data,
        message: message,
...
Query.passwordCheck = curry2(
    (err, password, hash) =>
        new Promise((resolve, reject) =>
            bcrypt
                .compare(password, hash)
                .then((result) => (result ? resolve(result) : reject(err))),
        ),
);
...
Query.getColumns = curry2(
    (tb, col, data) => QUERY1`SELECT ${COLUMN(...col)} FROM ${TB(tb)} WHERE ${EQ(data)}`,
);
Query.getById = curry((tb, id) => QUERY1`SELECT * FROM ${TB(tb)} WHERE ${EQ({ id })}`);
Query.getByIdColumns = curry2(
    (tb, col, id) => QUERY1`SELECT ${COLUMN(...col)} FROM ${TB(tb)} WHERE ${EQ({ id })}`,
);
...
Query.updateWhere = curry2(
    (tb, data, condition) =>
        QUERY1`UPDATE ${TB(tb)} ${SET(data)} WHERE ${EQ(condition)} RETURNING *`,
);
