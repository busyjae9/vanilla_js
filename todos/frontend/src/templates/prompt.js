...
<div class='prompt'>
    <div class='prompt__body'>
            ...
            data?.value
                ...
                <form action='' class='prompt__body__values'>
                    <div class='prompt__body__values__row'>
                        ...
                        <input 
                            class='prompt__body__values__row__content' 
                            name='date' 
                            placeholder='날짜 선택' type='text' 
                            onfocus="(this.type='date')" 
                            onblur="(this.type='text')" 
                            value='${new Date(data.value.date).toDateInputValue()}'>
                    </div>
                    <div class='prompt__body__values__row'>
        ...
        <div class='prompt__body__buttons'>${strMap(Prompt.mkButtonTmp, data.buttons)}</div>
    </div>
</div>
`;
...
Prompt.pop = (data) =>
    new Promise((resolve) => {
        data = data?.buttons ? data : { ...data, buttons: [{ msg: '확인', class: 'ok' }] };
        ...
        go(
            ...
            map(
                $on('click', (e) =>
                    go(e.currentTarget, tap($closest('.prompt'), $remove), (currentTarget) => {
                        resolve({
